...
from dolmos.logs import debug, error
from dolmos.ui import ui
from dolmos.utils import format_size, synchronized

# not defaulting to latest because of upstream halmos issue #492
...
SOLVER_CACHE_DIR = Path.home() / ".dolmos" / "solvers"
...
# (HALMOS_ALLOW_DOWNLOAD is still honored for existing setups)
ALLOW_DOWNLOAD_VAR = "DOLMOS_ALLOW_DOWNLOAD"
LEGACY_ALLOW_DOWNLOAD_VAR = "HALMOS_ALLOW_DOWNLOAD"
...
def download_allowed(solver: SolverInfo, download_info: DownloadInfo) -> bool:
    ...
    bypass = any(
        os.environ.get(var, "false").lower() in ["true", "1"]
        for var in (ALLOW_DOWNLOAD_VAR, LEGACY_ALLOW_DOWNLOAD_VAR)
    )
    if bypass:
        return True
...
def get_solver_command(solver_name: str) -> list[str]:
    ...
    if not solver_info:
        ...
        # solvers not managed by dolmos can be accessed with the low-level
...
# python -m dolmos.solvers <solver_name>
