...
from dolmos import env
from dolmos.assertions import assert_cheatcode_handler
...
from dolmos.calldata import (
    FunctionInfo,
    get_abi,
    mk_calldata,
)
from dolmos.constants import MAX_MEMORY_SIZE
...
class Prank:
    ...
    def lookup(self, to: BV) -> PrankResult:
        ...
        assert_address(to)
        if self and to not in [svm_cheat_code.address, hevm_cheat_code.address]:
            result = self.active
            if not self.keep:
                self.stopPrank()
            return result
...
def _get_contract_name(ex, arg) -> tuple[str | None, str | None]:
    ...
    if not (code := ex.code.get(addr)):
        raise DolmosException(f"createCalldata: not contract account: {hexify(addr)}")
...
def create_generic(ex, bits: int, var_name: str, type_name: str) -> BitVecRef | ByteVec:
    ...
    label = f"dolmos_{var_name}_{type_name}_{uid()}_{ex.new_symbol_id():>02}"
...
def create_uint(ex, arg, name: str | None = None, **kwargs):
    ...
    if bits > 256:
        raise DolmosException(f"bitsize larger than 256: {bits}")
...
def create_uint256_min_max(ex, arg, name: str | None = None, **kwargs):
    ...
    if min_value > max_value:
        raise DolmosException(f"expected min={min_value} <= max={max_value}")
...
class svm_cheat_code:
    ...
    def handle(sevm, ex, arg: ByteVec, stack) -> list[BitVecRef]:
        funsig = int_of(extract_funsig(arg), "symbolic svm cheatcode")
        if handler := svm_cheat_code.handlers.get(funsig):
            result = handler(ex, arg, sevm=sevm, stack=stack)
            return result if isinstance(result, list) else [result]

        error_msg = f"Unknown svm cheat code: function selector = 0x{funsig:0>8x}, calldata = {hexify(arg)}"
        raise DolmosException(error_msg)
