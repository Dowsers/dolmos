...
from dolmos.logs import warn
from dolmos.solvers import SOLVERS, get_solver_command
from dolmos.utils import parse_time
...
class ConfigSource(IntEnum):
    ...
    # contract-level annotation (e.g. @custom:dolmos --some-option)
    contract_annotation = 3

    # function-level annotation (e.g. @custom:dolmos --some-option)
...
class Config:
    """Configuration object for dolmos.
    ...
    config: str = arg(
        ...
        metavar="FILE",
        global_default=lambda: os.path.join(os.getcwd(), "dolmos.toml"),
        global_default_str="ROOT/dolmos.toml",
    )
...
def _create_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        # use the name the command was invoked with
        prog=program_name(),
        epilog="For more information, see https://github.com/Dowsers/dolmos",
    )
...
def program_name() -> str:
    """The name the command was invoked with (`dolmos` for `python -m dolmos`)."""
    name = os.path.basename(sys.argv[0])
    return "dolmos" if not name or name.endswith(".py") else name
...
# python -m dolmos.config ARGS > dolmos.toml
def main():
    ...
    lines = ["# dolmos configuration, generated by `python -m dolmos.config`"]
