...
from dolmos.bytevec import ByteVec, SymbolicChunk
from dolmos.calldata import FunctionInfo
from dolmos.cheatcodes import Prank, hevm_cheat_code, svm_cheat_code
from dolmos.config import Config as DolmosConfig
from dolmos.console import console
from dolmos.constants import MAX_ETH, MAX_MEMORY_SIZE
...
class CallOutput:
    ...
    error: EvmException | DolmosException | None = None
...
class CallContext:
    ...
    def is_stuck(self) -> bool:
        ...
        return data is None or isinstance(error, DolmosException)

    def get_stuck_reason(self) -> DolmosException | None:
        """
        Returns the first internal error encountered during the execution of the call.
        """
        if isinstance(self.output.error, DolmosException):
...
class Exec:  # an execution path
    ...
    def halt(
        ...
        if output.data is not None:
            raise DolmosException("output already set")

        if data is not None and not isinstance(data, ByteVec):
            raise DolmosException(f"invalid output data {data}")
    ...
    def sha3_data(self, data: Bytes) -> Word:
        ...
        if sha3_hash is not None:
            ...
            if sha3_hash_int == 0 or sha3_hash_int > 2**256 - 2**64:
                ...
                raise DolmosException(error_msg)
...
class SEVM:
    ...
    def __init__(self, options: DolmosConfig, fun_info: FunctionInfo) -> None:
    ...
    def resolve_address_alias(
        ...
        if not allow_branching and tail:
            raise DolmosException(f"multiple aliases exist: {hexify(target)}")
    ...
    def call(
        ...
        def call_unknown() -> None:
            ...
            # svm cheat code
            elif to == svm_cheat_code.address:
                exit_code = ONE
                ret = svm_cheat_code.handle(self, ex, arg, stack)
            ...
            for idx, ret_ in enumerate(ret_lst):
                if not isinstance(ret_, ByteVec):
                    raise DolmosException(f"Invalid return value: {ret_}")
    ...
    def create(
        ...
        if op == OP_CREATE:
            ...
            raise DolmosException(f"Unknown CREATE opcode: {op}")
            ...
            try:
                ...
                # Rest of the less frequent opcodes
                    ...
                    if account_alias is not None:
                        ...
                        # NOTE: the codesize of the svm cheatcode should be non-zero to pass the extcodesize check
                        ...
                        codesize = (
                            ...
                            if account
                            in [hevm_cheat_code.address, svm_cheat_code.address]
                            else ZERO
                        )
                    ...
                    raise DolmosException(f"Unsupported opcode {mnemonic(opcode)}")
