...
from dolmos.config import Config, TraceEvent
from dolmos.exceptions import DolmosException
from dolmos.mapper import DeployAddressMapper, Mapper
...
def render_trace(context: CallContext, file=sys.stdout) -> None:
    ...
    if config is None:
        raise DolmosException("config not set")
    ...
    for trace_element in context.trace:
        match trace_element:
            ...
            case _:
                raise DolmosException(f"unexpected trace element: {trace_element}")
