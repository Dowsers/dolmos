...
class BuildOut(metaclass=SingletonMeta):
    ...
    def get_by_name(self, contract_name: str, filename: str = None) -> dict:
        ...
        Raise a DolmosException if the contract is not found, or cannot be uniquely determined.
        ...
        if not mapping:
            raise DolmosException(f"{contract_name} is not found")
        ...
        if not result:
            raise DolmosException(f"{contract_name} is not found in {filename}")
