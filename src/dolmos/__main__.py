...
from dolmos.calldata import FunctionInfo, get_abi, mk_calldata
from dolmos.cheatcodes import snapshot_state
...
from dolmos.flamegraphs import CallSequenceFlamegraph, call_flamegraph, exec_flamegraph
...
from dolmos.mapper import BuildOut, DeployAddressMapper
from dolmos.processes import ExecutorRegistry, ShutdownError
...
from dolmos.traces import (
    render_trace,
    rendered_address,
    rendered_call_sequence,
    rendered_trace,
)
from dolmos.ui import suspend_status, ui
...
def with_devdoc(args: DolmosConfig, fn_sig: str, contract_json: dict) -> DolmosConfig:
...
def with_natspec(
    args: DolmosConfig, contract_name: str, contract_natspec: str
) -> DolmosConfig:
...
def with_vyper_defaults(args: DolmosConfig) -> DolmosConfig:
    """
...
def load_config(_args) -> DolmosConfig:
...
def mk_solver(args: DolmosConfig, logic="QF_AUFBV", ctx=None) -> Solver:
...
def deploy_test(ctx: FunctionContext, sevm: SEVM) -> Exec:
    ...
    # sanity check
    if len(exs) != 1:
        raise DolmosException(f"constructor: # of paths: {len(exs)}")
    ...
    if output.error is not None or not returndata:
        raise DolmosException(f"constructor failed: {output.error=} {returndata=}")
...
def run_target_contract(
    ...
    for fun_sig, fun_selector in target_selectors:
        ...
        try:
            ...
            tx_origin = mk_addr(
                f"dolmos_tx_origin_{id_str(addr)}_{uid()}_{ex.new_symbol_id():>02}"
            )

            # create a symbolic msg.sender
            msg_sender = mk_addr(
                f"dolmos_msg_sender_{id_str(addr)}_{uid()}_{ex.new_symbol_id():>02}"
            )
            ...
            msg_value = BitVec(
                f"dolmos_msg_value_{id_str(addr)}_{uid()}_{ex.new_symbol_id():>02}",
                BitVecSort256,
            )
    ...
    for idx, pre_ex in enumerate(curr_exs):
        ...
        for addr in resolve_target_contracts(ctx.inv_ctx, pre_ex):
            ...
            for post_ex in post_exs:
                ...
                # ignore and report if dolmos-errored
                if subcall.is_stuck():
                    error(
                ...
                # update timestamp
                timestamp_name = f"dolmos_block_timestamp_depth{depth}_{uid()}"
...
def run_test(ctx: FunctionContext) -> TestResult:
    ...
    dolmos.traces.config_context.set(args)
...
def execute_simple_getter(
    ...
    # sanity check
    if len(exs) != 1:
        raise DolmosException(f"{fun_info.sig}: # of paths: {len(exs)}")
    ...
    if output.error is not None or not returndata:
        raise DolmosException(f"{fun_info.sig}: {output.error=} {returndata=}")
...
def resolve_target_contracts(ctx: InvariantTestingContext, ex: Exec) -> set[Address]:
    ...
    if not resolved_target_contracts:
        raise DolmosException("No target contracts available.")
...
def _main(_args=None) -> MainResult:
    ...
    if args.version:
        print(f"{program_name()} {metadata.version('dolmos')}")
        return MainResult(0)
    ...
    if args.trace_memory:
        import dolmos.memtrace as memtrace
...
# entrypoint for the `dolmos` script
def main() -> int:
    exitcode = _main().exitcode
    return exitcode


# entrypoint for `python -m dolmos`
